...
set C {i in POSN} := {u in V :
    sum {j in POSN : j < i} posn_size[j] < u 
        <= sum {j in POSN : j <= i} posn_size[j]};
...
set NEG_EDGES in Efull := 
    union {(i,j) in negative_pairs}
        {(u,v) in C[i] cross C[j] : costE[u,v] != 0.0};

set POS_EDGES in Efull :=
    union {(i,j) in positive_pairs} C[i] cross C[j];
...
#
# Handle multiple solutions if requested
#
...
#
# Exclude previous solutions, if requested
#
